...
function SmartLoot.SetAutorollQualityFilter(self, arg1)
	UIDropDownMenu_SetSelectedID(SmartLoot_OptionsFrame_AutorollListHeader_Quality, self:GetID());
	SmartLoot.AutorollList.QualityFilter = arg1;
end
	...
	for i = 1, SmartLoot.AutorollList.PageSize, 1 do
		...
		if(hide) then
			...
			if(info == nil) then
				...
				local disenchant = getglobal(frame:GetName().."_Disenchant");
				...
				disenchant:SetChecked(info.roll == SmartLoot.Roll.Disenchant);
...
function SmartLoot.SetOption(option, value)
	-- widget OnLoad/OnValueChanged handlers can fire before ADDON_LOADED
...
function SmartLoot.GetAutorollListData(qualityFilter, nameFilter)
	...
	for name, info in SmartLoot.Sorted(SmartLoot_Autoroll) do
		local add = true;
		...
		if(nameFilter and string.find(string.lower(name), nameFilter, 1, true) == nil) then
			add = false;
		end
